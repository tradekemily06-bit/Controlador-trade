...
(function(){
  ...
  save?.addEventListener('click',async()=>{try{const f=validate(input?.files?.[0]);const response=await fetch('/api/ecosystem-image?kind='+encodeURIComponent(kind?.value||'profile'),{method:'POST',headers:{'Content-Type':f.type},body:f});const payload=await response.json();if(!response.ok)throw Error(payload.error||'Falha ao salvar imagem.');status.textContent='Imagem salva no ecossistema e disponível nos outros dispositivos conectados ao mesmo runtime.';status.className='muted ok';await loadExisting();}catch(e){status.textContent=e.message;status.className='muted danger';}});
  loadExisting();
})();