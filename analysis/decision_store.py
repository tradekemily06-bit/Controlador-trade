...
class DecisionStore:
    ...
    SQL identifiers are deliberately static. Runtime values are always bound
    as SQLite parameters; no caller-controlled value is interpolated into SQL.
    """
    ...
    _SELECT_SQL = (
        "SELECT decision_id, created_at, symbol, timeframe, signal, score, "
        "confirmed, reason, execution_allowed, outcome "
        "FROM decisions ORDER BY created_at ASC"
    )
    _INSERT_SQL = (
        "INSERT OR REPLACE INTO decisions "
        "(decision_id, created_at, symbol, timeframe, signal, score, confirmed, "
        "reason, execution_allowed, outcome) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    )
