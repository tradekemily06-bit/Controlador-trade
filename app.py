...
def _file_response(start_response, path: Path, content_type: str, request_id: str, environ) -> list[bytes]:
    ...
    if script_nonce:
        ...
        if path == WEB_DIR / "index.html":
            ...
            notification_mount = (notification_html + f'<script nonce="{script_nonce}">{notification_js}</script>').encode("utf-8")
            onboarding_mount = (onboarding_html + f'<script nonce="{script_nonce}">{onboarding_js}</script>').encode("utf-8")
...
def application(environ, start_response):
    ...
    try:
        ...
        if path == "/api/outcome" and method == "POST":
            data = _read_json(environ); record = SERVICE.record_outcome(str(data.get("decision_id", "")), str(data.get("outcome", "")))
