...
from .decision_engine import FinalDecision, DecisionResult
...
class HistoricalValidationEngine:
    """Executa o fluxo de decisão em histórico sem expor candles futuros.

    A função de decisão recebe somente o histórico disponível até o candle
    corrente. Quando uma decisão for EXECUTAR, a liquidação histórica é
    delegada ao BacktestEngine existente, preservando sua semântica de TP/SL.
    """
    ...
    def run(
        ...
        *,
        ...
        for index, signal in zip(entry_indexes, entry_signals):
            trade = self.backtest_engine._simulate_trade(
                candles=candles,
                entry_index=index,
                signal=signal,
                entry_price=candles[index].close,
                take_profit=(
                    candles[index].close + take_profit_distance
                    if signal == Signal.COMPRA
                    else candles[index].close - take_profit_distance
                ),
                stop_loss=(
                    candles[index].close - stop_loss_distance
                    if signal == Signal.COMPRA
                    else candles[index].close + stop_loss_distance
                ),
            )
            trades.append(trade)
