...
class RuntimeConfig:
    ...
    REAL execution is not a runtime configuration capability. Any attempt to
    construct a REAL runtime, or to mark REAL as enabled through configuration,
    is rejected fail-closed. Future REAL release authority must come from the
    dedicated release/authorization path, never from config.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.real_enabled:
            raise ValueError("habilitação REAL não pode ser criada por configuração.")
