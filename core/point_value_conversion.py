...
def convert_price_movement(spec: ContractSpec, request: PointMoneyRequest) -> PointMoneyResult:
    ...
    try:
        ...
        return PointMoneyResult(
            ConversionStatus.INVALID, spec.broker, spec.symbol, spec.asset_class,
            spec.unit_kind, Decimal("0"), None, None, None,
            ("movement_or_quantity",),
        )

    if movement < 0 or quantity <= 0 or spec.unit_size <= 0:
        return PointMoneyResult(
            ConversionStatus.INVALID, spec.broker, spec.symbol, spec.asset_class,
            spec.unit_kind, movement, None, None, None,
            ("positive_movement_and_quantity_required",),
        )

    if spec.value_per_unit is None:
        return PointMoneyResult(
            ConversionStatus.NEEDS_DATA, spec.broker, spec.symbol, spec.asset_class,
            spec.unit_kind, movement, None, None, None,
            ("value_per_unit",),
        )

    units = movement / spec.unit_size
    value = spec.value_per_unit
    assumptions: list[str] = []

    if spec.account_currency and spec.quote_currency and spec.account_currency != spec.quote_currency:
        if request.fx_to_account is None or request.fx_to_account <= 0:
            return PointMoneyResult(
                ConversionStatus.NEEDS_DATA, spec.broker, spec.symbol, spec.asset_class,
                spec.unit_kind, movement, units, None, None,
                ("fx_to_account",),
            )
        value = value * request.fx_to_account
        assumptions.append("account_currency_conversion_supplied_by_market_data_contract")

    impact = units * quantity * value
    return PointMoneyResult(
        ConversionStatus.AVAILABLE, spec.broker, spec.symbol, spec.asset_class,
        spec.unit_kind, movement, units, value, impact, (), tuple(assumptions)
    )
