...
class LeverageProfile:
    ...
    def validate(self) -> None:
        ...
        if self.capital_available is not None and (not self.capital_available.is_finite() or self.capital_available < 0):
            raise ValueError("capital_available_must_be_non_negative")
        if self.maximum_loss is not None and (not self.maximum_loss.is_finite() or self.maximum_loss < 0):
