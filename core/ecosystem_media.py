...
class EcosystemImage:
    ...
    def validate(self) -> None:
        if not isinstance(self.image_id, str) or not self.image_id.strip() or not isinstance(self.storage_reference, str) or not self.storage_reference.strip():
            raise ValueError("image_identity_required")
        if self.mime_type not in ALLOWED_IMAGE_MIME_TYPES:
            raise ValueError("unsupported_image_type")
        if not isinstance(self.size_bytes, int) or self.size_bytes <= 0 or self.size_bytes > MAX_IMAGE_BYTES:
        ...
        if ".." in self.storage_reference.replace("\\", "/").split("/"):
            raise ValueError("unsafe_storage_reference")
