...
from core.mt5_candle_analysis_service import analyze_mt5_candidates_with_default_candle_method
...
class FakeProvider:
    def fetch_market_data(self, request: BrokerMarketDataRequest):
        return (
            Candle(datetime(2026, 9, 12, 12, 0, tzinfo=timezone.utc), 100, 103, 99, 102, 100),
            Candle(datetime(2026, 9, 12, 12, 5, tzinfo=timezone.utc), 102, 108, 101, 107, 120),
        )
...
def test_default_method_wires_ranked_assets_to_candle_evaluator():
    ...
    results = analyze_mt5_candidates_with_default_candle_method(
        candidates,
        boundary,
        timeframe="5m",
        limit=20,
