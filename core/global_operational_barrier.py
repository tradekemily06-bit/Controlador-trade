...
class GlobalOperationalBarrier:
    ...
    def evaluate(self) -> BarrierDecision:
        ...
        try:
            ...
            return BarrierDecision(BarrierStatus.UNKNOWN, f"estado de segurança indisponível: {type(exc).__name__}", ("safety-state",))
        if blocking:
    ...
    def remediate(self) -> tuple[RemediationResult, ...]:
        ...
        for component in self._components:
            ...
            if component.remediation_mode is not RemediationMode.AUTO_SAFE:
                results.append(RemediationResult(component.name, component.remediation_mode, False, False, component.detail)); continue
            if component.repair is None:
                results.append(RemediationResult(component.name, RemediationMode.AUTO_SAFE, False, False, "reparo seguro não configurado")); continue
            try:
                component.repair()
            except Exception as exc:
