...
def build_ic_markets_mt5_demo_analysis_service(
    *,
    mt5_module: Any = None,
    timeframe: str = "5m",
    candle_limit: int = 50,
...
) -> Callable[[], tuple[MT5AssetAnalysis, ...]]:
    """Build a read-only IC Markets MT5 DEMO analysis service."""
    ...
    if analysis_limit is not None and (
        not isinstance(analysis_limit, int) or isinstance(analysis_limit, bool) or analysis_limit < 0
    ):
