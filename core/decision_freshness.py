...
class DecisionFreshnessPolicy:
    ...
    def validate(self, created_at: datetime, *, now: datetime) -> str | None:
        ...
        if created_utc > now_utc + timedelta(seconds=self.max_future_skew_seconds):
            return "decisão rejeitada: timestamp está no futuro"
        if (now_utc - created_utc).total_seconds() > self.max_age_seconds:
            return "decisão expirada: a avaliação ficou velha demais para execução; nova análise obrigatória"
        return None
