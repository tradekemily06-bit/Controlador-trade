...
class RuntimeHealthMonitor:
    """Read-only runtime health monitor."""
    def __init__(self, *, ledger: ExecutionLedger, lifecycle: ExecutionLifecycleStore) -> None:
        if not isinstance(ledger, ExecutionLedger): raise ValueError("ledger inválido.")
        if not isinstance(lifecycle, ExecutionLifecycleStore): raise ValueError("lifecycle inválido.")
        self.ledger, self.lifecycle = ledger, lifecycle
    ...
    def assess(self) -> RuntimeHealth:
        ...
        if unknown: return RuntimeHealth(HealthState.BLOCKED, ledger_entries, pending, unknown, "execuções UNKNOWN requerem reconciliação")
        if pending: return RuntimeHealth(HealthState.ATTENTION, ledger_entries, pending, 0, "existem execuções PENDING para verificação")
