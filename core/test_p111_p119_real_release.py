...
def _gateway(registry, ledger):
    return RealExecutionGateway(
        BrokerAdapterGateway(registry), ledger, FakeRiskStateProvider(_risk_state())
    )
...
def test_real_unknown_is_persisted_and_retry_is_blocked(tmp_path: Path):
    ...
    safety = _safety(auth)
    first = gateway.execute(broker="fake", request_id="unknown-1", request=_request(), authorization=auth, admission=admission, safety=safety, snapshot=_snapshot())
    ...
    second = restored.execute(broker="fake", request_id="unknown-1", request=_request(), authorization=auth, admission=admission, safety=safety, snapshot=_snapshot())
...
def test_real_unknown_requires_explicit_reconciliation_before_resolution(tmp_path: Path):
    ...
    admission = _admission(auth)
    safety = _safety(auth)
    result = gateway.execute(broker="fake", request_id="unknown-2", request=_request(), authorization=auth, admission=admission, safety=safety, snapshot=_snapshot())
...
def test_real_reserved_after_restart_is_unknown_and_reconcilable(tmp_path: Path):
    ...
    admission = _admission(auth)
    safety = _safety(auth)
    result = gateway.execute(broker="fake", request_id="crashed", request=_request(), authorization=auth, admission=admission, safety=safety, snapshot=_snapshot())
    assert result.status == RealGatewayStatus.UNKNOWN
    assert adapter.calls == 0
...
def test_real_accepted_without_external_id_is_unknown(tmp_path: Path):
    ...
    admission = _admission(auth)
    safety = _safety(auth)
    result = gateway.execute(broker="fake", request_id="missing-id", request=_request(), authorization=auth, admission=admission, safety=safety, snapshot=_snapshot())
    assert result.status == RealGatewayStatus.UNKNOWN
...
def test_real_gateway_blocks_changed_authoritative_risk_before_dispatch(tmp_path: Path):
    ...
    auth = _authorization()
    result = gateway.execute(broker="fake", request_id="risk-changed", request=_request(), authorization=auth, admission=_admission(auth), safety=_safety(auth), snapshot=_snapshot())
    assert result.status == RealGatewayStatus.BLOCKED
    assert adapter.calls == 0
...
def test_real_gateway_blocks_provider_failure_without_leaking_detail(tmp_path: Path):
    class BrokenProvider:
        def current_risk_state(self):
            raise RuntimeError("SECRET_RISK_PROVIDER_DETAIL")
    ...
    auth = _authorization()
    result = gateway.execute(broker="fake", request_id="risk-provider-fails", request=_request(), authorization=auth, admission=_admission(auth), safety=_safety(auth), snapshot=_snapshot())
    assert result.status == RealGatewayStatus.UNKNOWN
    assert "SECRET_RISK_PROVIDER_DETAIL" not in result.message
