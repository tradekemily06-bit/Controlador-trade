...
def _authorization(request_id="req-1", symbol="TEST", broker_id="fake", adapter_id="fake-adapter"):
    return RealExecutionAuthorization(
        authorization_id="auth", audit_id="audit-1", broker_id=broker_id,
...
def _admission(request_id="req-1", symbol="TEST", broker_id="fake", adapter_id="fake-adapter"):
...
def _request(request_id="req-1", symbol="TEST"):
...
def test_real_adapter_timeout_is_unknown_not_rejected(tmp_path: Path):
    ...
    result = _gateway(ledger_path, adapter).execute(
        ...
        authorization=_authorization("timeout-1"), admission=_admission("timeout-1"),
        safety=_safety(), snapshot=_snapshot(),
    )
...
def test_real_replay_after_restart_is_blocked_without_second_dispatch(tmp_path: Path):
    ...
    first_result = _gateway(path, adapter).execute(
        ...
        authorization=_authorization("restart-1"), admission=_admission("restart-1"),
        safety=_safety(), snapshot=_snapshot(),
    )
...
def test_real_changed_risk_is_blocked_before_broker_dispatch(tmp_path: Path):
    ...
    result = gateway.execute(
        ...
        authorization=_authorization("risk-change"), admission=_admission("risk-change"), safety=safety,
        snapshot=_snapshot(),
    )
...
def test_real_global_barrier_blocks_before_dispatch(tmp_path: Path):
    ...
    result = _gateway(tmp_path / "ledger.json", adapter, barrier).execute(
        ...
        authorization=_authorization("barrier-1"), admission=_admission("barrier-1"),
        safety=_safety(), snapshot=_snapshot(),
    )
    assert result.status == RealGatewayStatus.BLOCKED
    assert adapter.calls == 0
...
def test_real_unknown_retry_never_reaches_adapter_again(tmp_path: Path):
    ...
    first = _gateway(path, adapter).execute(
        ...
        authorization=_authorization("unknown-retry"), admission=_admission("unknown-retry"),
        safety=_safety(), snapshot=_snapshot(),
    )
