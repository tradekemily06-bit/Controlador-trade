...
def test_p111_p116_p117_p119_positive_flow(tmp_path: Path):
    ...
    gateway = RealExecutionGateway(BrokerAdapterGateway(registry), ledger, ExecutionLifecycleStore(tmp_path / "lifecycle.json"), KillSwitch(), FakeReconciler("test", executed=True))
...
def test_real_gateway_blocks_without_active_authorization(tmp_path: Path):
    ...
    adapter = FakeAdapter()
    registry.register("fake", adapter)
    gateway = RealExecutionGateway(BrokerAdapterGateway(registry), ExecutionLedger(tmp_path / "ledger.json"), ExecutionLifecycleStore(tmp_path / "lifecycle.json"), KillSwitch(), FakeReconciler("test", executed=True))
...
def test_real_unknown_is_persisted_and_retry_is_blocked(tmp_path: Path):
    ...
    restored = RealExecutionGateway(BrokerAdapterGateway(registry), ExecutionLedger(tmp_path / "ledger.json"), ExecutionLifecycleStore(tmp_path / "lifecycle.json"), KillSwitch(), FakeReconciler("test", executed=True))
...
def test_real_unknown_reconciles_only_from_trusted_external_evidence(tmp_path: Path):
    ...
    ledger = ExecutionLedger(tmp_path / "ledger.json")
    lifecycle = ExecutionLifecycleStore(tmp_path / "lifecycle.json")
    gateway = RealExecutionGateway(BrokerAdapterGateway(registry), ledger, lifecycle, KillSwitch(), FakeReconciler("test", executed=True))
    auth = _authorization()
    admission = _admission(auth)
    safety = _safety(auth)
...
def test_real_reserved_after_restart_is_unknown_and_reconcilable(tmp_path: Path):
    ...
    adapter = FakeAdapter()
    registry.register("fake", adapter)
    gateway = RealExecutionGateway(BrokerAdapterGateway(registry), ExecutionLedger(path), ExecutionLifecycleStore(tmp_path / "lifecycle.json"), KillSwitch(), FakeReconciler("test", executed=True))
    auth = _authorization()
    admission = _admission(auth)
    safety = _safety(auth)
...
def test_real_reservation_creates_pending_lifecycle_before_dispatch(tmp_path: Path):
    ...
    gateway = RealExecutionGateway(
        ...
        KillSwitch(), FakeReconciler("test", executed=True),
    )
    auth = _authorization()
    admission = _admission(auth)
    safety = _safety(auth)
...
def test_real_gateway_rejects_admission_bound_to_different_broker_or_audit(tmp_path: Path):
    ...
    lifecycle = ExecutionLifecycleStore(tmp_path / "lifecycle.json")
    gateway = RealExecutionGateway(
        BrokerAdapterGateway(registry), ledger, lifecycle, KillSwitch(), FakeReconciler("test", executed=True)
    )
