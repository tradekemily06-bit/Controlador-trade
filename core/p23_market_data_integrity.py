    ...
    def assess(
        ...
        *,
        ...
        if not items or any(not isinstance(c, Candle) or not c.is_valid() for c in items):
            return MarketDataIntegrityReport(MarketDataHealth.INVALID, len(items), expected_interval_seconds, 0, False, "dados de mercado inválidos")
        timestamps = tuple(c.timestamp for c in items)
        if any(a >= b for a, b in zip(timestamps, timestamps[1:])):
            return MarketDataIntegrityReport(MarketDataHealth.INVALID, len(items), expected_interval_seconds, 0, False, "timestamps fora de ordem ou duplicados")
        gap_count = 0
        ...
        if stale:
            return MarketDataIntegrityReport(MarketDataHealth.STALE, len(items), expected_interval_seconds, gap_count, True, "último candle está desatualizado")
        if gap_count:
            return MarketDataIntegrityReport(MarketDataHealth.GAP, len(items), expected_interval_seconds, gap_count, False, "há lacunas na sequência de candles")
        return MarketDataIntegrityReport(MarketDataHealth.HEALTHY, len(items), expected_interval_seconds, 0, False, "dados de mercado íntegros")
