...
class DemoFlow:
    ...
    def __init__(
        ...
        *,
        ...
        demo_coordinator: DemoExecutionCoordinator,
    ...
    def run(
        ...
        *,
        ...
        execution_result = self.demo_coordinator.execute(
            config=config,
        ...
        execution = (
            execution_result.gateway.execution
            if execution_result.gateway is not None
            else None
        )
        ...
        self.audit_logger.record(
            AuditEvent(
                ...
                data={
                    "accepted": execution is not None and execution.accepted,
                    "external_id": execution.external_id if execution is not None else None,
                    "message": (
                        execution.message
                        if execution is not None
                        else execution_result.readiness.reason
                    ),
                    "quality_level": quality.level.value,
                    "quality_score": quality.score,
                    "readiness": execution_result.readiness.reason,
                },
            )
        )
