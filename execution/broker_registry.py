...
class BrokerRegistry:
    ...
    def register(self, name: str, adapter: BrokerAdapter, *, adapter_id: str | None = None) -> None:
        ...
        stable_id = adapter_id.strip() if isinstance(adapter_id, str) else normalized
        if not stable_id:
            raise BrokerRegistryError("adapter_id não pode ser vazio.")
        if stable_id in self._adapter_ids.values():
            raise BrokerRegistryError(f"adapter_id já registrado: {stable_id}")
        self._adapters[normalized] = adapter
        self._adapter_ids[normalized] = stable_id
