...
def test_paper_executor_rejects_real_mode():
    executor = PaperExecutor()
    result = executor.execute(make_request(mode=ExecutionMode.REAL))
...
def test_paper_executor_rejects_non_finite_amount():
    executor = PaperExecutor()
    result = executor.execute(make_request(amount=float("nan")))
    assert result.accepted is False
    assert executor.executions() == ()
