...
def test_external_id_attached_before_acceptance_failure_remains_reconcilable(tmp_path):
    ...
    # The broker side effect may already exist. The authoritative local state
    # must therefore be UNKNOWN, never RESERVED, after acceptance persistence
    # fails; recovery can reconcile using the durable external_id.
    assert entry.status is ExecutionLedgerStatus.UNKNOWN
    assert entry.external_id == "broker-attach-failure"
    assert lifecycle.get("attach-failure").state is ExecutionLifecycleState.UNKNOWN

    recovery = gw.reconcile_unknown(
        "attach-failure",
        broker="fake",
        authorization=auth(),
        reconciliation_boundary=ExternalOrderReconciliationBoundary(),
    ...
    assert ledger.status("attach-failure") is ExecutionLedgerStatus.RECONCILED_EXECUTED
    assert lifecycle.get("attach-failure").state is ExecutionLifecycleState.ACCEPTED
