...
def test_real_gateway_blocks_new_request_while_other_request_is_uncertain(tmp_path: Path):
    registry = BrokerRegistry()
    adapter = MissingExternalIdAdapter()
    registry.register("fake", adapter)
    ledger = ExecutionLedger(tmp_path / "ledger.json")
    ledger.reserve("old-uncertain")
    gateway = RealExecutionGateway(BrokerAdapterGateway(registry), ledger)
    authorization = RealExecutionAuthorization("auth", "audit", "fake", "adapter", True, True)
    admission = RealAdmissionBoundary().admit(
        admission_id="adm", audit_id="audit", audit_verified=True,
    ...
    result = gateway.execute(
        broker="fake", request_id="new-request", request=request,
        authorization=authorization, admission=admission, safety=safety,
    )
...
class ExplicitlyUncertainAdapter:
    def is_available(self):
        return True

    def execute(self, request):
        return ExecutionResult(False, "broker response ambiguous", None, True)


def test_explicit_uncertain_execution_never_becomes_rejected(tmp_path: Path):
    registry = BrokerRegistry()
    registry.register("fake", ExplicitlyUncertainAdapter())
    ledger = ExecutionLedger(tmp_path / "ledger.json")
    gateway = RealExecutionGateway(BrokerAdapterGateway(registry), ledger)
    authorization = RealExecutionAuthorization("auth", "audit", "fake", "adapter", True, True)
    admission = RealAdmissionBoundary().admit(
    ...
    result = gateway.execute(
        broker="fake", request_id="explicit-uncertain", request=request,
        authorization=authorization, admission=admission, safety=safety,
    )

    assert result.status == RealGatewayStatus.UNKNOWN
    assert ledger.status("explicit-uncertain") is ExecutionLedgerStatus.UNKNOWN
