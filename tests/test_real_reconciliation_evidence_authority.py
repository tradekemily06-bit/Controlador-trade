...
def test_gateway_accepts_external_id_when_no_persisted_external_id_exists(tmp_path: Path):
    path = tmp_path / "ledger.json"
    ledger = ExecutionLedger(path)
    ...
    assert ExecutionLedger(path).status("req-id") is ExecutionLedgerStatus.RECONCILED_EXECUTED
