...
from integration.mt5_demo_analysis_service import build_ic_markets_mt5_demo_analysis_service
...
class FakeMT5:
    ...
    def copy_rates_from_pos(self, symbol, timeframe, start_pos, count):
        assert timeframe == self.TIMEFRAME_M5 and start_pos == 1
        return [{"time": 1000, "open": 100.0, "high": 101.0, "low": 99.0, "close": 100.5, "tick_volume": 10, "real_volume": 10}, {"time": 1060, "open": 100.5, "high": 102.0, "low": 100.0, "close": 101.8, "tick_volume": 12, "real_volume": 12}]
...
def test_service_rejects_invalid_configuration():
    ...
    with pytest.raises(ValueError): build_ic_markets_mt5_demo_analysis_service(analysis_limit=-1)
    with pytest.raises(TypeError): build_ic_markets_mt5_demo_analysis_service(evaluator=object())
